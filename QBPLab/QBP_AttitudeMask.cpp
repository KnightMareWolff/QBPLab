...
CQBPAttitudeMask::CQBPAttitudeMask()
{
    iQBPOctave    = 5;
}
...
bool CQBPAttitudeMask::CalculaScores()
{
    ...
    for (int i = 1; i < CQBPMatrix::GetSizeX(); i++)
    {
        ...
        for (int j = 1; j < CQBPMatrix::GetSizeY(); j++)
        {
            ...
            fSimScore = CalculaSimilaridade(i, j, bSimMatch);
            ...
            iQBPDiagonal = CQBPMatrix::GetVal(i - 1,j - 1, iTempDir, bTempMatch) + fSimScore;
            ...
            //atribui o score e direcao apos o calculo de Max.
...
int CQBPAttitudeMask::CalculaSimilaridade(int pIndiceX, int pIndiceY, bool &pMatch)
{
    int resultado=0;
    ...
    if (CalculaDistancia(vQBP_IBufferA[pIndiceX - 1],vQBP_IBufferB[pIndiceY - 1])==0)
    {
        ...
        iQBPMatch     =   vQBP_IBufferA[pIndiceY - 1].iQBP_Octave;
...
int CQBPAttitudeMask::CalculaDistancia(stQBPMusicBuffer  pSinalA ,stQBPMusicBuffer pSinalB)
{
